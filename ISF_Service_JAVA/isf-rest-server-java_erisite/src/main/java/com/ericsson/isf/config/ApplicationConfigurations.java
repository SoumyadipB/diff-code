...
public class ApplicationConfigurations {
	...
	private void init(String environmentName) {
		...
		try {
			...
			String fileLocation = StringUtils.EMPTY.equals(environmentName) ? rootPath 
					: filePath + "properties/" + environmentName;
			LOG.info("File Location is --------------------------------------------------------------"+fileLocation);
			PropertiesConfiguration configuration = new PropertiesConfiguration(fileLocation + "/application.properties");
...
//			String operatingSystem = System.getenv("CONFIG_OS");
			 String appConfigUrl = System.getenv(CONFIG_URL);
			LOG.info("appConfigUrl from  CONFIG_URL is =========== "+appConfigUrl);
			if(!appConfigUrl.endsWith("/")) 
			{
				appConfigUrl=appConfigUrl+"/";
		    }
			String appUrl=appConfigUrl+environmentName+"/application";
			
			LOG.info("properties file url from  CONFIG server is =========== "+appUrl);
			...
			LOG.info("!!!!!!!!!!!!!!!Inside Catch block of  appConfigurations !!!!!!!!!!!!!!!!");
