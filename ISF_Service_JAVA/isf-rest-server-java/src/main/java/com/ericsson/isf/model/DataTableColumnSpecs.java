...
public class DataTableColumnSpecs {
	...
	private void prepareColumnSpecs(HttpServletRequest request, int i) {
		String colname=request.getParameter("columns["+ i +"][data]");
		if(colname.contains(".")){
			colname=colname.substring(colname.lastIndexOf('.')+1);
		}
		this.setData(colname);
		...
		String search=request.getParameter("columns["+ i +"][search][value]");
		this.setSearch(	(search==null||"".equals(search))?search:"%"+search+"%"	);
